...
class Token
{
    ...
    private function resolveNft()
    {
        ...
        if ($request->isError()) {
            ...
            throw new PradoException("Error {$request->status()} processing request for token {$this->token_id} in contract {$this->contract} at {$this->blockchain} blockchain. Response: " . $request->body());
        }
